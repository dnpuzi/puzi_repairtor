  ...
  <div class="adminPcLoginSearch">
    ...
    <div class="container">
      <Form :inline="true" class="demo-form-inline">
        ...
        <FormItem>
          <Button type="primary" @click="searchByUserName">查询</Button>
        </FormItem>
        <FormItem label="状态：">
          <Select v-model="searchCategoryName" placeholder="请选择" @change="searchByCategoryName">
            <Option
            v-for="item in this.$store.state.searchCategoryNames"
            :key="item.value"
            :label="item.label"
            :value="item.value">
            </Option>
        ...
        <Button type="primary" :loading="true" v-show="isLoading">加载中</Button>
      </Form>
      <Table
        :data="devList"
        v-show="devList.length"
        border
        style="width: 100%">
        <TableColumn
        prop="id"
...
export default {
  ...
  methods: {
    ...
    searchByUserName() {
      if(this.searchUserName == '') {
        ...
        this.$axios.post('http://repaire.dnpuzi.com/Admin/index/searchByUserName', qs.stringify(data))
        .then(res => {
    ...
    searchByCategoryName() {
      console.log(this.searchCategoryName);
      let data = {
        status: this.searchCategoryName
      }
      this.isLoading = true;
      this.$axios.post('http://repaire.dnpuzi.com/Admin/index/searchByCategoryName', qs.stringify(data))
      .then(res => {
        if(res.data.success == true) {
          this.searchUserName = '';
          // console.log(res);
  ...
  mounted() {
    var oHead = document.getElementsByTagName('head').item(0); 
      ...
      oScript.src="http://10.253.89.44:8000//CLodopfuncs.js"; 
